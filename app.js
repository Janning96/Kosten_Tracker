// Version & PDF worker
const APP_VERSION = 'v2025-08-18-5';
pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdn.jsdelivr.net/npm/pdfjs-dist@3.11.174/build/pdf.worker.min.js';

// Basic health
function health(msg){ const el = document.getElementById('health'); el.textContent = msg; el.hidden = !msg; }
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('ver').textContent = `Offline‑fähig • IndexedDB • PWA • ${APP_VERSION}`;
});

// ---- DB (IndexedDB) ----
const DB_NAME = 'kosten-tracker-db'; const DB_VERSION = 7; let db;
function openDB(){return new Promise((res,rej)=>{const r=indexedDB.open(DB_NAME,DB_VERSION);
  r.onupgradeneeded=e=>{const db=e.target.result;
    if(!db.objectStoreNames.contains('transactions')){const s=db.createObjectStore('transactions',{keyPath:'id',autoIncrement:true});s.createIndex('date','date');s.createIndex('dedupe','dedupe_key',{unique:true});}
    else{const s=r.transaction.objectStore('transactions'); if(!s.indexNames.contains('date')) s.createIndex('date','date'); if(!s.indexNames.contains('dedupe')) s.createIndex('dedupe','dedupe_key',{unique:true});}
...
function parseAmountToCents(raw){
  let s = String(raw).replace(/\s|€|\u00A0/g,'').replace(/\u2212/g,'-').replace(/\./g,'').replace(',', '.');
  let sign = 0;
  if (/[\+\-–-]$/.test(s)) { const last=s.slice(-1); if(last==='+')sign=+1; if(last==='-'||last==='–')sign=-1; s=s.slice(0,-1); }
  ...
  if (sign===0) sign = -1; // N26 default: negative
  return value*sign;
}
...
function chooseCounterparty(lines){
  const cand=[...lines].reverse().map(s=>s.replace(/\u00A0/g,' ').replace(/\u2212/g,'-').trim());
  for(let raw of cand){
    if(!raw) continue;
    const lr = raw.toLowerCase();
    if (CP_STOP_WORDS.some(sw => lr.startsWith(sw))) continue;
    let s = stripPrefixes(raw); s = stripTrailingNoise(s);
    const brand = brandMap(s); if (brand) return brand;
    const star = s.split('*'); if(star.length>1 && star[1].trim()) s = star.slice(1).join('*').trim();
    const tokens = s.split(/\s+/), good = tokens.filter(t=>!looksLikeIdToken(t));
    const candidate = (good.length ? good.join(' ') : s).trim();
    if (candidate && /[A-Za-zÄÖÜäöüß]/.test(candidate)) return candidate;
  }
...
function groupIntoLines(items){
  // tolerant grouping by Y (±3 px)
  const sorted = items.map(it => ({y: it.transform[5], x: it.transform[4], s: it.str})).sort((a,b)=>a.y-b.y||a.x-b.x);
  const tol = 3; const lines=[]; let bucket=[]; let currentY=null;
  for(const it of sorted){
    if(currentY===null || Math.abs(it.y-currentY)<=tol){ bucket.push(it); if(currentY===null) currentY=it.y; }
    else{ lines.push(bucket.slice()); bucket=[it]; currentY=it.y; }
  }
  if(bucket.length) lines.push(bucket);
  return lines.map(row => row.sort((a,b)=>a.x-b.x).map(t=>t.s).join(' ').replace(/\s+/g,' ').trim()).filter(Boolean);
}
...
function detectBank(text){
  ...
  const bar=['umsatzübersicht','belegdatum','valutadatum','betrag (eur)','barclays']; const n26=['n26','vorläufig','space','unterkonto','kontoumsätze','kontoauszug','spaces zusammenfassung'];
  const score=k=>k.reduce((s,x)=>s+(t.includes(x)?1:0),0); const sb=score(bar), sn=score(n26); if(sb===0&&sn===0) return ''; return sn>=sb?'n26':'barclays';
}
...
function parseBarclays(lines){
  ...
  for(const ln of lines){
    ...
    if(m){ const[, be,val,descRaw, amtRaw, trailing] = m; const desc = descRaw.trim();
      if(/gutschrift\s+manuelle\s+lastschrift/i.test(desc)) continue; // ignore rule
      const amountStr = amtRaw + (trailing||'');
      let cents; try{ cents = parseAmountToCents(amountStr,{bank:'barclays',sectionHint}); }catch{ continue; }
...
// N26 parser (full doc, combined lines + counterparty)
function parseN26(lines){
  ...
  const reCombined=/wertstellung\s+(\d{2}\.\d{2}\.\d{4})\s+(\d{2}\.\d{2}\.\d{4})\s+([+\-−]?\d{1,3}(?:\.\d{3})*,\d{2})€?/i;
  ...
  for(let raw of lines){
    ...
    // Combined line
    const cmb=line.match(reCombined);
    if(cmb){
      const verbuch=cmb[2]; const amount=cmb[3];
      const desc=chooseCounterparty(buf.length?buf:[section]);
      if(/barclays/i.test(desc)) { buf=[]; pending=null; continue; } // ignore N26->Barclays
    ...
    const wm=line.match(reWertstellung); if(wm){ pending = wm[1]; continue; }

    const pm=line.match(rePosting);
    if(pm && pending){ const verbuch=pm[1]; const amount=pm[2]; const desc=chooseCounterparty(buf.length?buf:[section]);
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  }catch(e){ health('Datenbank-Initialisierung fehlgeschlagen. Bitte Seite neu laden.'); console.error(e); }
  ...
  $('#btn-clear-filter')?.addEventListener('click', () => { ['flt-month','flt-from','flt-to','flt-source','flt-cat'].forEach(id=>{const el=document.getElementById(id); if(el) el.value='';}); refreshTransactions(); });
  ...
  $('#pdf-input')?.addEventListener('change', () => {
    const n = $('#pdf-input').files?.length || 0;
    $('#import-log').textContent = n ? `${n} PDF ausgewählt` : '';
  });

  // Analyse Exporte
  $('#btn-ana-apply')?.addEventListener('click', refreshCharts);
  $('#btn-ana-clear')?.addEventListener('click', () => { ['ana-month','ana-from','ana-to'].forEach(id=>{const el=document.getElementById(id); if(el) el.value='';}); refreshCharts(); });
...
async function renderCategorySelects(){
  ...
  const selects = [$('#flt-cat'), $('#cat-series')].filter(Boolean);
  selects.forEach(sel => {
    sel.innerHTML = '';
    if(sel===$('#flt-cat')){ const o=document.createElement('option'); o.value=''; o.textContent='Alle Kategorien'; sel.appendChild(o); }
...
async function parseSelectedPDFs(){
  ...
  $('#import-log').textContent = 'Analysiere…'; preview = [];
  for(const f of input.files){
    ...
    }catch(e){ console.warn('PDF parse error', e); $('#import-log').textContent = 'Fehler beim PDF‑Import: ' + (e.message||e); }
  }
...
function renderPreview(){
  const tb = document.querySelector('#preview-table tbody'); if(!tb) return; tb.innerHTML='';
  preview.forEach(it => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${srcLabel(it.source)}</td><td>${formatDE(it.date)}</td><td>${escapeHTML(it.description||'')}</td><td class="num">${fromCents(it.amount_cents)}</td><td>Neu</td>`;
    tb.appendChild(tr);
  });
}
async function commitPreview(){
  if(!preview.length) return;
  const existing = await getAll('transactions'); const keys = new Set(existing.map(t=>t.dedupe_key));
  const add = []; for(const it of preview){ it.category||=UNDEF; if(!keys.has(it.dedupe_key)) add.push(it); }
  await putMany('transactions', add);
  clearPreview(); alert(`Import abgeschlossen: ${add.length} neue Transaktionen.`);
  await refreshTransactions(); await refreshCharts();
}
function clearPreview(){ preview=[]; const tb=document.querySelector('#preview-table tbody'); if(tb) tb.innerHTML=''; $('#btn-commit-import').disabled=true; $('#import-log').textContent=''; }
...
async function refreshTransactions(){
  ...
  list.forEach(t => {
    ...
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${formatDE(t.date)}</td><td>${escapeHTML(t.description||'')}</td><td>${t.category||UNDEF}</td><td>${srcLabel(t.source)}</td><td class="num">${fromCents(t.amount_cents)}</td><td></td>`;
...
async function refreshCharts(){
  const all = await getAll('transactions');
  const month=$('#ana-month')?.value; const from=$('#ana-from')?.value || null; const to=$('#ana-to')?.value || null;
  ...
  trendChart?.destroy();
  trendChart = new Chart(document.getElementById('ch-trend'), { type:'bar',
  ...
  const sumByCat={}; data.forEach(t => { sumByCat[t.category||UNDEF] = (sumByCat[t.category||UNDEF]||0) + t.amount_cents; });
  const labels = Object.keys(sumByCat).sort((a,b)=>a.localeCompare(b,'de')); const vals = labels.map(k => (sumByCat[k]||0)/100);
  barChart?.destroy(); barChart = new Chart(document.getElementById('ch-bar'), { type:'bar', data:{ labels, datasets:[{label:'Summe', data: vals}] },
  ...
  lineChart?.destroy(); lineChart = new Chart(document.getElementById('ch-line'), { type:'line', data:{ labels: lm, datasets:[{ label:selected, data: lv, tension:.25, pointRadius:4 }] },
    options:{ responsive:true, scales:{ y:{ ticks:{callback:v=>CURRENCY.format(v)} } }, plugins:{legend:{position:'bottom'}} } });
}
...
async function exportRangeExcel(){
  const from=$('#ana-from')?.value||null; const to=$('#ana-to')?.value||null; const month=$('#ana-month')?.value||null;
  const all=await getAll('transactions'); let data=all.slice(); if(month)data=data.filter(t=>yyyymm(t.date)===month); if(from||to)data=data.filter(t=>inRange(t.date,from,to)); data.sort((a,b)=>a.date.localeCompare(b.date));
  const rows=[['Datum','Beschreibung','Betrag (€)','Kategorie','Quelle']]; data.forEach(t=>rows.push([formatDE(t.date), t.description, (t.amount_cents/100).toFixed(2).replace('.',','), t.category||UNDEF, srcLabel(t.source)]));
...
async function exportAnalysisPDF(){
  ...
  const add = (id,y) => { const c=document.getElementById(id); const img=c.toDataURL('image/png',0.9); doc.addImage(img,'PNG',14,y,182,80); };
  add('ch-trend',22); doc.addPage(); add('ch-bar',16); add('ch-pie',100); doc.addPage(); add('ch-line',16);
